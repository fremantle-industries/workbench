defmodule BackOffice.BalanceSnapshots.SchedulerTest do
  ...
  @balance struct(BackOffice.Balance)
  @usd_quote_venue :venue_a

  setup do
    config =
      struct(
        BackOffice.BalanceSnapshots.Config,
        enabled: true,
        boot_delay_ms: 0,
        every_ms: 1,
  ...
  test "creates a snapshot after a boot delay & fires the 'after_snapshot' callback", %{
    ...
    with_mock BackOffice.BalanceSnapshots.Snapshot, create: fn _config -> {:ok, @balance} end do
      {:ok, _} = BackOffice.BalanceSnapshots.Scheduler.start_link(config: config, id: @test_name)
  ...
  test "creates a snapshot on a regular schedule & fires the 'after_snapshot' callback", %{
    ...
    with_mock BackOffice.BalanceSnapshots.Snapshot,
  ...
  test "doesn't create a snapshot when disabled", %{
    ...
    with_mock BackOffice.BalanceSnapshots.Snapshot, create: fn _config -> {:ok, @balance} end do
      {:ok, _} =
        BackOffice.BalanceSnapshots.Scheduler.start_link(config: disabled_config, id: @test_name)
  ...
  test "logs an event when there is an error taking the snapshot", %{config: config} do
    ...
    with_mock BackOffice.BalanceSnapshots.Snapshot,
      ...
      assert %BackOffice.BalanceSnapshots.Events.Error{} = event
