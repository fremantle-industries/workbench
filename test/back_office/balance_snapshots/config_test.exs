defmodule BackOffice.BalanceSnapshots.ConfigTest do
  use ExUnit.Case, async: true

  @valid_config struct(BackOffice.BalanceSnapshots.Config,
                  enabled: false,
  ...
  test "parses a snapshot config from the application environment" do
    assert {:ok, config} = BackOffice.BalanceSnapshots.Config.parse()
  ...
  test "returns an error when enabled is not present" do
    ...
    assert BackOffice.BalanceSnapshots.Config.parse(config_without_enabled) ==
  ...
  test "returns an error when btc_usd venue is not present" do
    ...
    assert BackOffice.BalanceSnapshots.Config.parse(config_without_venue) ==
  ...
  test "returns an error when btc_usd symbol is not present" do
    ...
    assert BackOffice.BalanceSnapshots.Config.parse(config_without_symbol) ==
  ...
  test "can parse boot_delay_ms" do
    ...
    assert {:ok, config} = BackOffice.BalanceSnapshots.Config.parse(config)
